...
int main(void)
{
    ...
    camera cam(90.0f, float(width) / float(heigth));

    float R = cos(M_PI / 4.0);
    hiTabel *list[2];
    list[0] = new sphere(vec3<float>(-R, 0.0f, -1.0f), R, new lambertian(vec3<float>(0.0f, 0.0f, 1.0f)));
    list[1] = new sphere(vec3<float>(R, 0.0f, -1.0f), R, new lambertian(vec3<float>(1.0f, 0.0f, 1.0f)));
